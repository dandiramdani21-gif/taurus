    ...
    <div className="flex flex-col lg:flex-row gap-6 p-4 md:p-6 bg-gray-50 min-h-screen">
      ...
      <div className="flex-1">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
          <h1 className="text-3xl font-semibold text-gray-900">Kasir HP</h1>
          <input
            type="text"
            placeholder="Cari brand, type, atau kode..."
            value={search}
                ...
                <tbody className="divide-y divide-gray-200">
                  {products.map((product) => (
                    <tr key={product.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-4">
                        <div className="font-medium">{product.brand} {product.type}</div>
                      </td>
                    ...
                    {item.isEditingCost ? (
                      <div className="flex gap-2 mt-1">
                        <input
                          ...
                          autoFocus
                        />
                        <button onClick={() => saveCostPrice(item.id, item.productId, item.tempCostPrice || 0)} className="px-4 bg-green-600 text-white rounded-xl">Save</button>
                        <button onClick={() => cancelEditCost(item.id)} className="px-4 bg-gray-200 rounded-xl">Batal</button>
                      </div>
                    ) : (
